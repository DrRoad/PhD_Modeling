...
class general:
    ...
    def TruckCountII():
        ...
        #edge_VehIDhistoryPD.to_csv(EdgeVehIDHistoryPANDAPATH)
        #edge_VehIDhistoryPD.to_csv(resetEdgeVehIDHistoryPANDAPATH)
        ...
        for edge_i in general.Belmont_Ave[:]:
            ...
            for vehID_k in Edge_i_VehIDs_lastStep_j:
                ...
                if vehID_k in edge_VehIDhistoryPD[str(edge_i_INDEX)]:
                    ...
                    notcar = int(notcar) + 1
                    edge_VehIDhistoryPD.loc[int(NOofvehIDsinList),str(edge_i_INDEX)]= vehID_k # fristOpenSpot = NOofvehIDsinList + 1
                    Edge_TOT_Trucks = int(edge_VehIDhistoryPD.at[str(edge_i_INDEX)-1,1])+int(notcar)
                    Edge_TOT_Trucks_Current_Period = notcar

                    edge_VehIDhistoryPD.loc[int(str(edge_i_INDEX))-1,int(edge_VehIDhistoryPD.shape[1]-1)] = Edge_TOT_Trucks_Current_Period
                    edge_VehIDhistoryPD.loc[int(str(edge_i_INDEX))-1,1] = Edge_TOT_Trucks
                    edge_VehIDhistoryPD.to_csv(EdgeVehIDHistoryPANDAPATH)
